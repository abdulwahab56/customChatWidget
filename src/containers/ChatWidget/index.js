...
const ChatWidget = ({
    ...
    const { primaryColor, description, region, apiGateway, contactFlowId, instanceId, enableAttachments, name: configName, username: configUsername, contactAttr: configContactAttr } = useAppConfig();
    ...
    const getNameAndUserName = (formData = {}, initialPropertiesName = '', initialPropertiesUsername = '') => {
        ...
        if (name.includes(chatWidgetDefaults.REFER_INDICATOR)) {
            error(`name field has value ${initialPropertiesName} which is not referenced correct`);
            name = chatWidgetDefaults.NAME;
        }
        if (username.includes(chatWidgetDefaults.REFER_INDICATOR)) {
            error(`username field has value ${initialPropertiesUsername} which is not referenced correct`);
    ...
    const getContactAttrsForContactFlow = (name = '', username = '', dataFromInputForm = {}, initialPropertiesContAttrs, initialPropertiesName, initialPropertiesUsername) => {
        ...
        if (dataFromInputForm) {
            log(`Appending ${JSON.stringify(dataFromInputForm)} to contact attributes`);
        ...
        if (initialPropertiesContAttrs) {
            log(`Appending ${JSON.stringify(initialPropertiesContAttrs)} to contact attributes`);
    ...
    const initializeChat = () => {
        const { name, username } = getNameAndUserName(dataFromInputForm, configName, configUsername);
        log('Name and username to initiate a chat connection: ', { name, username });

        let contactAttrs = getContactAttrsForContactFlow(name, username, dataFromInputForm, configContactAttr, configName, configUsername);
